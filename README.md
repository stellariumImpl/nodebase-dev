...
当前流程能力集中在“触发器 + HTTP 请求”的最小可用闭环：

- **工作流程列表**：可查看、搜索、分页、创建与删除流程。
- **流程编辑器**：可编辑流程名称、添加节点、连线并保存。
- **手动触发（Trigger）**：点击 “Execute workflow” 即可启动流程。一个流程仅允许一个手动触发器。
- **Google Form 触发（Trigger）**：通过 Google Form 的 Apps Script 发送 Webhook，触发表单提交后的流程执行。
...
- **从表单触发自动处理**：Google Form 提交后自动进入流程，例如写入系统、通知、或数据同步。

## 5. 可扩展方向（建议的节点能力）

如果你觉得现有节点偏“单一”，可以优先补齐下面这些能力，让流程更像“自动化平台”：

- **触发器类**
  - **Webhook Trigger**：对外提供通用 Webhook（不仅是 Google Form）。
  - **Schedule/Cron Trigger**：定时触发（比如每日、每小时）。
  - **Database Trigger**：监听数据变化（如新增/更新）。
- **执行节点类**
  - **Delay/Wait**：等待一段时间或等待条件满足。
  - **Condition/Branch**：支持 if/else、路由分支，形成不同执行路径。
  - **Transform/Mapper**：对数据进行清洗、映射、格式转换。
  - **Notification**：邮件、Slack、钉钉/飞书等通知节点。
  - **Storage/Cache**：写入数据库、文件存储或缓存。
